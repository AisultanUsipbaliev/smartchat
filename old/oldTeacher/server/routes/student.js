...
module.exports = async function(req, res) {
	try {
		startLog('student', {body: req.body, index: req.cookies.SAI});
		if (!req.body.method) throw new Error("NOT FOUND METHOD PROVIDED!");
		let result = null;
		switch (req.body.method.toUpperCase()) {
			case "GET": result = await controller.getStudent(req);				break;
		...
		await errorLog('student', {body: req.body, index: req.cookies.SAI}, err)
