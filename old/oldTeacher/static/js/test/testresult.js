	...
	let searchText = top.location.search;
	let all_id = searchText.split('?');
	if(all_id[1]){
		let student_id = all_id[1].split('=')[1];
		let test_id = all_id[2].split('=')[1];
		if(student_id || test_id ){
			POST('/result', `method=GET-RESULT&&test_id=${test_id}&&student_id=${student_id}`, function(res, status){
...
function addQuestionerResult(data, result, layout, num)
{
	...
	let ans = false;
	let correct = getInt(data.correct);
	let studentanswer = getInt(result);
	...
	for(let i=0; i<data.variants.length; i++) {
		if(i+1 == correct){
			...
			if(ans) corAns++;
		}else if(ans == false && i+1 == studentanswer){
...
function addTestControlPanelerResult(layout, layoutbody, id, data)
{
	if (counter_div % 2 == 0) {
		...
		div.querySelector('#escape').addEventListener('click', (e)=>{top.location.href = '/result'});
