...
//Функция закрашивает дни в черный (ВЫХОДНЫЕ)
	function getWeekend(req)
	{
		if(req.status == 200)
		{
			...
			{
				if(res.body[i].nday!=0) mas.push(res.body[i].nday);
				else mas.push(7);
			}
...
//Функция закрашивает дни в краыный (ЗАНЯТЫЕ)
	function getRedDays(req)
	{
		let res = JSON.parse(req.response);
		let body = res.mas;
		let trs 	= calendar.children;
		...
		for(let i = 0; i < trs.length; i++)
		{
			...
			for(let j = 0; j < tds.length; j++)
			{
				if(tds[j].innerHTML != '')
				{
					let date = new Date( year +' ' + month + ' ' + tds[j].innerHTML);

					for(let z = 0; z < body.length; z++)
					{
						let dt = new Date(body[z].start);
				...
				for(let j = 0; j < tds.length; j++)
				{
					if(tds[j].innerHTML != '')
					{
						...
						for(let z = 0; z < body.length; z++)
						{
							...
							let y1 	= dt.getUTCFullYear();
							let y2 	= date.getUTCFullYear();

							let m1	= dt.getUTCMonth();
							let m2	= date.getUTCMonth();

							let d1 	= dt.getUTCDate();
							let d2 	= date.getUTCDate()+1;

							if(y1 == y2 && m1 == m2 && d1 == d2)
							{
								// console.log(tds[j].onclick())
								tds[j].style.color = "#5353ff";
								if(jer != j){
									tds[j].addEventListener('click', (e)=> {showWorkDay(e.target, date.getDate())});
								}
								jer = j;
							}
						}
...
//Функция вывода рабочего дня
	function showWorkDay(target, nday)
	{
		...
		let begin = new Date(year + '-' + mounth + '-' + day + ' 00:00').valueOf();
		let end = new Date(year + '-' + mounth + '-' + day + ' 24:00').valueOf();
		// let begin;
		// let end;
		// if(UTCFunc() > 0){
		// 	begin = new Date(year + '-' + mounth + '-' + (Number(day) - 1) + ' ' + UTCFunc() +':00').valueOf();
		// 	if(!begin){
		// 		day = 31;
		// 		mounth--;
		// 		while(true){
		...
		// 		}
		// 	}
		// 	end = new Date(year + '-' + mounth + '-' + day + ' ' + UTCFunc() + ':00').valueOf()
		// }
		// else if(UTCFunc() < 0){
		// 	begin = new Date(year + '-' + mounth + '-' + day + ' ' + (24 + UTCFunc()) +':00').valueOf();
		// 	end = new Date(year + '-' + mounth + '-' + (Number(day) + 1) + ' ' + (24 + UTCFunc()) + ':00').valueOf()
		// 	if(!end){
		// 		day = 1;
		// 		mounth++;
		// 		end = new Date(year + '-' + mounth + '-' + (Number(day) + 1) + ' ' + UTCFunc() +':00').valueOf();
		// 	}
		// }else{
		// 	begin = new Date(year + '-' + mounth + '-' + day + ' 00:00').valueOf();
		// 	end = new Date(year + '-' + mounth + '-' + day + ' 24:00').valueOf()
		// }
		
		POST('/chart', 'method=GET&&begin=' + begin + '&&end=' + end, (req)=>
			{
				let res = JSON.parse(req.response);
				if(req.readyState == 4)
				{
					...
					if(req.status == 200)
					{
						...
						day_name.innerHTML = getDayName(new Date(body[0].start).getDay());
						...
						for(let i = 0; i<body.length; i++)
						{
							if(body[i].group_type == 1)
							{
								...
								group_name.innerHTML = deleteSlash(body[i].group_name); 
								...
								body[i].start_time = new Date(body[i].start).getUTCHours();
								let minuts = new Date(body[i].start).getUTCMinutes();
								if(minuts === 0) minuts = '00';
								body[i].nday = new Date(body[i].start).getUTCDay();
								body[i].finish_time = new Date(body[i].finish).getUTCHours();
								if (body[i].finish_time == 0) body[i].finish_time = 24;
								let time = ConvertToView(body[i], UTCdifference)
								
								for(let j = 0; j < time.length; j++)
								for(let q = 0; q < j; q++)
									time[i].finish_time = time[q].finish_time
								start_time.innerHTML = time[0].start_time + ':'+ minuts;
								tr.appendChild(start_time);
								//Конец занятия
								let finish_time = document.createElement('td');
								finish_time.classList.add('finish_time');
								finish_time.innerHTML = time[0].finish_time + ':'+ minuts;
								tr.appendChild(finish_time);
						...
						for(let i = 0; i<body.length; i++)
						{
							if(body[i].group_type != 1)
							{
								...
								a.href = 'group?id=' + body[i].group_id;
								a.innerHTML = body[i].group_name;
					...
					{
						notifier('На сегодня нет занятий!', 'blue');
					}
				}
			});
	}
//Функция создает календарь
	...
	{	
		...
		POST('/graph', 'method=GET', getWeekend);
		POST('/chart', 'method=GET', getWorkDays);
		POST('/chart', 'method=GET-FULL&&firstday='+firstday+'&&lastday='+lastday, getRedDays);
	}
// СКРИПТ
