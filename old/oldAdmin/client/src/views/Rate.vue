<template>
  <div class="container-fluid">
          ...
          <div class="card__body">
            ...
            <div class="table-responsive" v-dragscroll>
              <table class="table table-bordered table-hover">
                ...
                <tbody>
                  <tr v-for="(rate, index) in rates" :key="index" :title="rate.rate_title">
                    ...
                    <td v-else>-</td>
                    <td class="rateContent" v-if="rate.rate_content">{{rate.rate_content}}</td>
                    <td v-else>-</td>
                    <td v-if="rate.lessons">{{rate.lessons}}</td>
                    <td v-else>-</td>
                    <td v-if="rate.rate_cost !== ''">
                    	<span class="oldCost" v-if="rate.sale">{{rate.oldCost}}Kzt<br></span>
                    ...
                    <td  class="table__btn-container">
                      ...
                      <button class="btn" title="Удалить"  @click="console.log('delete(index)')">
                        <fai icon="trash-alt"/>
                      </button>
                    </td>
                  </tr>
              ...
              <div class="row table-footer-btn-block m-0" v-if="pagination">
                <button class="col-12 btn mt-2" @click="console.log('getData(true)')">Показать еще</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <router-link :to="{ name:'RateDetail', params:{id:'new'} }"><fai icon="plus" class="newRate"/></router-link>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getData(flag) {
      ...
      this.$http.post(`/rate`, `method=GET-ALL-RATES&column=${this.column}&order=${this.order}&from=${this.from}&limit=${this.limit}&text=${this.search}`)
      .then(res => {
    ...
    changeActivate(rate){
    	...
    	this.$http.post(`/rate`, `method=CHANGE-ACTIVATE&rate_id=${rate.rate_id}&newStat=${rate.active}`)
      .then(res => {
