          ...
          <div class="card__body settings">
            <div class="row">
              ...
              <div class="settings__item col-lg-6">
                ...
                <input id="group" class="input" type="number" :class="[configData.groups_check?'':'alert-danger']" v-model.trim="configData.groups">
              </div>
              <div class="settings__item col-lg-6">
              ...
              <div class="settings__item col-lg-6">
                ...
                <input id="max_students" class="input" type="number" :class="[configData.students_check?'':'alert-danger']" v-model.trim="configData.students">
              </div>
              <div class="settings__item col-lg-6">
	          ...
	          <div class="row">
	            ...
	            <div v-if="editPass" class="settings__item col-lg-6">
	              ...
	              <input id="old_pass" class="input" type="text" v-model.trim="oldPass" :class="[oldPass_check?'':'alert-danger']">
	            </div>
	            <div v-if="editPass" class="settings__item col-lg-6">
	              <label class="settings__item" for="first_new_pass">Новый пароль:</label>
	              <input id="first_new_pass" class="input" type="text" v-model.trim="newPass" :class="[newPass_check?'':'alert-danger']">
	            </div>
	            <div v-if="editPass" class="col-lg-6 m-0 p-0"></div>
	            <div v-if="editPass" class="settings__item col-lg-6">
	              <label class="settings__item" for="second_new_pass">Подтвердите пароль:</label>
	              <input id="second_new_pass" class="input" type="text" v-model.trim="confirmNewPass" :class="[confirmNewPass_check?'':'alert-danger']">
	            </div>
	          </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    sendForm: function () {
        ...
        this.$http.post(`/account`, `method=UPDATE-CONFIG&data=${JSON.stringify(data)}&passwords=${JSON.stringify(passwords)}`)
        .then(response => {
