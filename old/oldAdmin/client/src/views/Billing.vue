      ...
      <div class="col-lg-5">
        ...
        <div class="card__body" style="padding-bottom: 0">
          <div class="row">
            ...
            <div :class="cur_time ? 'col-sm-10' : 'col-sm-12'" class="text-cent">{{cur_name}}</div>
            <div class="col-sm-1 text-cent" v-if="cur_time" @click="switchDate(1)">></div>
          </div>
          <div class="row">
...
export default {
  ...
  methods: {
    ...
    getData(flag) {
      ...
      this.$http.post(`/billing`, `method=GET-BILLING&column=${this.column}&order=${this.order}&from=${this.from}&limit=${this.limit}&text=${this.search}&start=${this.getTimeValue(this.start)}&end=${this.getTimeValue(this.end)}`)
      .then(res => {
        if (res.status == 200) {
    ...
    getInfo(){
      this.getData();
      this.$http.post(`/billing`, `method=GET-INFO&start=${this.getTimeValue(this.start)}&end=${this.getTimeValue(this.end)}`)
      .then((res)=>{
        if(res.status === 200){
          let data = res.data.data;
          this.count = this.getSpaceInCount(data.amount || 0);
          this.operations = this.getSpaceInCount(data.length);
        }else{
    ...
    getGraphInfo(start, end){
      this.$http.post(`/billing`, `method=GET-INFO&start=${start}&end=${end}`)
      .then((res)=>{
        if(res.status === 200){
          let data = res.data.data;
