  ...
  <div class="content">
    <div class="container-fluid">   
      ...
      <div class="row" v-if="groupList.length==0">
        ...
        <div class="col-lg-8 m-xl-0 m-sm-0 m-md-0 p-3">
          <div class="card">
            <div>
              ...
              <div class="card__body profile-more-info ">
                ...
                <div class="profile-more-info__text">
                  <small>Статус </small>
                    <span class="m-0 badge badge-success" v-if="!teacher.role" @click="editTeacherRole()">Младший преподаватель</span>
                    <span class="m-0 badge badge-info" v-else @click="editTeacherRole()">Старший преподаватель</span>                        
                </div>
                <hr>
                <div class="profile-more-info__text">
                  <small>Уровень: </small>
...
export default {
  ...
  computed: {
    avaUrlComputed: function () { return `${ this.$commonPhotoUrl }${ this.teacher.ava }` }
  },
  methods: {
    getTeacherAllInfo: function () {
      this.$http.post(`/teacher`, `method=GET-TEACHER-ALL-INFO&teacher_id=${this.teacher_id}`)
      .then(async (response) => {
    ...
    cancelShedule(){
      ...
      console.log(this.oldShedule)
      this.shedule = this.oldShedule;
      console.log(this.shedule)
    },
    editShedule(){
      this.editingShedule = true; 
      this.oldShedule = this.shedule;
      console.log(this.oldShedule)
    },
    getShedule(lieData, flag){
      ...
      for(let i = 0; i < data.length; i++){
        if(!flag) data[i] = this.$toClient([data[i].start, data[i].finish ? data[i].finish : data[i].end, data[i].nday])
        else data[i] = this.$toServer([data[i].start, data[i].finish ? data[i].finish : data[i].end, data[i].nday])
      ...
      if(ending.length < 7)
        for(let i = 0; i < 7; i++){
          ...
          for(let j = 0; j < ending.length; j++)
            ...
            if(nope) ending.push({start: 'нет', end: 'нет', nday: i})
        }
    ...
    saveShedule(){
      ...
      this.$http.post(`/teacher`, `method=UPDATE-TEACHER-SHEDULE&teacher_id=${this.teacher_id}&graph=${JSON.stringify(shedule)}`)
      .then(response => {
    ...
    editTeacherRole: function () {
      this.$http.post(`/teacher`, `method=EDIT-ROLE&role=${this.teacher.role}&teacher_id=${this.teacher_id}`)
      .then(response => {
    ...
    resetThePassword: function () {
      this.$swal({
        ...
        preConfirm: (login) => {
          return this.$http.post(`/teacher`, `method=UPDATE-TEACHER-PASSWORD&teacher_id=${ this.teacher_id }`)
    ...
    groupChartDateParse: function (data) {
      if (data.length>0) {
        for (let i = 0; i < data.length; i++) {
    ...
    chartPreSaveEdit: function(index) {
      ...
      if (this.groupList[index].temp_start_day.length == 0 &&
        ...
        if (start_date < finish_date) {
          ...
          this.$http.post(`/student`, `method=UPDATE-STUDENT-CHART&chart_id=${data.chart_id}&start=${data.start}&finish=${data.finish}&lesson=${data.lesson}`)
          .then(async (response) => {
            if (response.data.status == 200) {
    ...
    chartDelete: function (index) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (login) => {
          return this.$http.post(`/student`, `method=DELETE-STUDENT-CHART&chart_id=${chart_id}`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    toggleEmailOn: function (event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/teacher`, `method=UPDATE-TEACHER-MAIL-ON&teacher_id=${ this.teacher_id }&value=${ this.teacher.mailOn ? 0 : 1 }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    toggleSmsOn: function (event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/teacher`, `method=UPDATE-TEACHER-SMS-ON&teacher_id=${ this.teacher_id }&value=${ this.teacher.smsOn ? 0 : 1 }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    togglePhoneActivated: function (event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/teacher`, `method=UPDATE-TEACHER-PHONE-ACTIVATE&teacher_id=${ this.teacher_id }&activate_phone=${ this.teacher.activated ? 0 : 1 }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    toggleEmailActivated: function (event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/teacher`, `method=UPDATE-TEACHER-EMAIL-ACTIVATE&teacher_id=${ this.teacher_id }&activate_email=${ this.teacher.is_active ? 0 : 1 }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    toggleBlocked: function (event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/teacher`, `method=BLOCK&teacher_id=${ this.teacher_id }&block=${ this.teacher.blocked ? 0 : 1 }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    levelSaveEdit: function () {
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/teacher`, `method=UPDATE-TEACHER-LEVEL&teacher_id=${ this.teacher_id }&level_id=${ this.temp_level_id }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
