...
exports.Sign = async function(req)
{
	...
	let pass = await model.getPassword();
...
exports.ChangePass = async function(req)
{
	...
	if(check)
	{
		...
		let res = await model.changePass(hash);
...
exports.GetConfig = async function(req)
{
	...
	let configs = await model.getConfig();
	
	let result;

	if (configs) {
		let teachers = await model.getSelectedTeacher();
...
exports.UpdateConfig = async function(req)
{
	...
	let result = await model.updateConfig(req.body.data.login, req.body.data.students, req.body.data.min_students, req.body.data.groups, req.body.data.quickTeacher, req.body.data.defaultCost, req.body.data.trialDefaultCost, req.body.data.goodTrial);
