...
function addQuestion(data, layout, num)
{
	...
	if(data.title){
		...
		title.style.width = "calc(100% + 2vw)";
...
// Функция добавляет аудио в чат
	function addAudioLevel(data, layout)
	{
		layout.innerHTML +=`
		<table>
			<tr style="display: flex; justify-content:center; align-items:center; position:relative">
				<td><img class="play_btn" src="/img/play.png" onclick="playAudio(this, '${data.audio}', ${data.id})"></td>
				<td class="audio_time" style="width:6vw" id="audio_time${data.id}">00:00</td>
				<td>
					<div class="audio_line" style="position:absolute" id="audio_line${data.id}" onclick="polosa(event, ${data.id})">
...
function addTestControlPanel(layout, layoutbody, id)
{
	if (div_count % 2 == 0) {
		...
		div.innerHTML = `
			<button class = 'test_button home-work__button' id = 'save'>Далее</button>
			<button class = 'test_button home-work__button' id = 'escape'>Отмена</button>
		`;


		div.querySelector('#escape').addEventListener('click', (e)=>{ 
			if(confirm('Вы действительно хотите закрыть? Вы можете потерять данные этой формы.'))
			{  closePopup(); window.cnt = 0; audio.stop(); }
		});
		...
		POST('/test', `method=RESULT-LEVEL&&index=${id}&&answers=${resultbody}`, (res)=>
			{
				...
				if (res.status == 200) 
				{
					...
					else 
					{
						...
						`
							<div class='levelSt'>
								<h2>Вы набрали ${window.cnt+res.rightAnswer} баллов из ${res.index}! \n Поздравляем ваш уровень ${res.lvl_name}!</h2>
								<button onclick ="top.location.href='/courses'" class="test_button home-work__button" style="margin: 0; padding: 2.5% 6%;">Хорошо</button>
							</div>
						`;
					}
