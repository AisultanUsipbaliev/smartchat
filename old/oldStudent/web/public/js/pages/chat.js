...
{ 
	let newMes = document.getElementById('navChat');
	if(newMes.children[2]){
		newMes.removeChild(newMes.children[2]);
	}
	...
	<div class = 'chat_container'>
		<div id = 'chat' class = 'chat'>
			<div class="bottom_arrow" id="bottom_arrow" onclick="downChat()">
				<img src="img/bottom-arrow.svg" alt="">
			</div>		
			...
			<div style="overflow: hidden;height: 90%; position:relative; transition: 0.3s" id="chat_msgs_block">
				<div style="overflow-x: hidden;overflow-y: auto; margin-right:-15%;height:100%; width:115%; padding-right:13.75%;" id="scrollBlockForChat">
					<div id = 'msgs' class = 'msgs'>
						<img src = '/img/logo.png' class = 'logotip' style = 'margin-top: 50px; width: 400px; max-height: 400px'>
					</div>
					<div id="scrollForChat" class="scroll" style="right:5%" onmousedown="personalScrollForChat()"></div>
				</div>
			</div>	
			...
			<div class = 'box' style='display:none'>
				...
				<div class = 'msg_div'>
					<textarea id = 'text' onkeypress="if (event.keyCode==13){ sendMes(event);}" autofocus placeholder="Введите сообщение" class = 'text' type="text"></textarea>
					<img src="/img/paperclip.png" title="Прикрепить файл" class  = 'screpka' onclick="document.getElementById('file').click()">
...
function getComplete(delivered){
	...
	if(delivered == 0){
		p.innerHTML = `<img src="/img/noncomplete.svg">`;
	}else{
		p.innerHTML = `<img src="/img/complete.svg">`;
	}
	return p;
}
...
// Функция добавляет аудио в чат
	function addAudio(data, layout)
	{
		...
		document.getElementById('nav_menu').addEventListener('click', ()=>
			{
...
function addRefHomeWork(data, layout){
	layout.innerHTML = `
		...
		<div class="hrefToTest">
			<img class="chat_picture" src="/img/if_book-write.svg" style="height:5vh">
		</div>
	`
}
...
function addRefTest(data, layout){
	layout.innerHTML = `
		<p class="student_name select_name">${data.refSender}</p>
		<p class="fileTitle">${data.refTitle}</p>
		<div class="hrefToTest">
			<img class="chat_picture" src="/img/if_notepad.svg" style="height:5vh">
		</div>
	`
}
...
// Обработка нажания на микрофон
	...
	document.getElementById('microfon').addEventListener('mousedown', function(e)
		{
			...
			ws.send(JSON.stringify({notice: 11, type: 2}));
			window.microfonInterval = setInterval(()=>{
				ws.send(JSON.stringify({notice: 11, type: 2}))
			}, 500)
			
			document.getElementById('microfon').addEventListener('mouseup', (e)=>
			{
				console.log(mediaRecorder.stop.toString())
				mediaRecorder.stop();
				animateMicrofon();
				clearTimeout(audioTimeout);
				clearInterval(window.microfonInterval)
			});
		});
...
//Функция запускает скролл
	...
	//Скролл захватом скроллбара в чате
		function personalScrollForChat(){
			chat_scrollPerson = true;
			document.onmousedown = function(){return false};
			let div = document.createElement('div');
			let rect = chat_block.getBoundingClientRect();
			...
			document.getElementsByClassName('wrapper')[0].appendChild(div);
				document.onmousemove = function(e){
					...
					if(e.target.id == 'scrollInvise'){
						let y = e.offsetY - (chat_scrollBar.clientHeight/2);
						chat_block.scrollTop = chat_StudentsCount * y;
					}else if(e.clientY <= rect.top){
						chat_block.scrollTop = 0;	
					}else if(e.clientY > rect.top){
						chat_block.scrollTop = msgs.offsetHeight;
					}
				}
			document.onmouseup = function(){
				document.body.style.cursor = "auto";
				document.onmousedown = function(){chat_scrollPerson = false;};
				document.onmousemove = function(){chat_scrollPerson = false;};
