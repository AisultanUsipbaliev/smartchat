function getSchedulePage()
{
	...
	<div class="calendar_div container">
			<img id = 'refresh' title="Обновить" src="/img/fresh.png">
			<h2 class='navi'>
	            <a id = 'goleft' title="Предыдущий месяц" class="change_month" style='margin-right: 4vh;'>
	            ...
	            <a id = 'goright' title="Следующий месяц" class="change_month" style='margin-left: 4vh;'>
		...
		POST('/chart', 'method=GET&&begin=' + begin + '&&end=' + end, (res, stat)=>
			{				
				...
				if(stat == 200)
				{
					...
					//Заголовок
					let daysClose = document.createElement('p');
					daysClose.id = 'daysClose';
					daysClose.innerHTML = 'Закрыть';
					div.appendChild(daysClose);
					...
					daysClose.addEventListener('click', function(){
					document.getElementById('day_chart').style.display = "none";
					});
