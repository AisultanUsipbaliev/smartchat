...
module.exports = async function(req, res) {
	...
	try {
		...
		let packet = await mailModel.getMail(99)	
		if(!packet) packet = {}

		let text 	= await replacer(packet.message, studentId)	
		let smsText = await replacer(packet.sms, studentId)

		sms(contact.phone, smsText)
		if(student.email) mail( text , student)
