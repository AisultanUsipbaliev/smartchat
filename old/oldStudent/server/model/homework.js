...
// Выборка
	exports.getHomework = async function(myId, id) {
		return (await SQL('select * from homework where student = ? and id = ?', [myId, id]))[0]
	}
...
// Изменение
	exports.addSolution = async function(myId, id, filePath) {
		return (await SQL('update homework set done = 1, filePath = ? where student = ? and id = ?', [filePath, myId, id])).affectedRows>0?true:false
	}