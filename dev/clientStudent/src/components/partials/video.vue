<!-- <template>
	<div :class="fullscreen ? 'video' : 'video__small'" @mousedown="playerMove" @touchstart="playerMove" :style="!fullscreen ? {top: top + 'px', left: left + 'px'} : ''" v-if="videoSrc">
		<fai icon="times" class="close" @click="closeVideo()" v-if="!fullscreen"/>
		...
		<div class="controllers" :class="!controllers ? 'controllers__hidden' : 'controllers__show'">
			<div class="left_ponel">
				...
				<div class="play" @mousemove="showVolume" @mouseover="hiddenVolume">
					...
					<div class="volume" :class="!changeVolume ? 'volume__hidden' : 'volume__show'" @mousedown="moveVolume" @touchstart="moveVolume" ref="volume">
		...
		<div class="timer" ref="timer" :class="!controllers ? 'controllers__hidden' : 'controllers__show'" @mousedown="moveVideo" @touchstart="moveVideo">
...
<script>
	...
	export default{
		data(){
			return{
				videoSrc: this.$store.getters['video/VIDEO'] || '',
				player: '',
				controllers: true,
				start: false,
				fullscreen: false,
		...
		methods:{
			closeVideo(){
				this.$store.dispatch('video/GET_VIDEO', {src: ''});
			},
			playerMove(e){
				if(!this.fullscreen && e.target == this.player){
	      ...
	      this.player.volume = this.volume;
				...
				document.onmousemove = (ev)=>{
					if(x + (onePercent / 10) < ev.pageX){
						x = ev.pageX;
						this.volume += 0.1
					}else if(x - (onePercent / 10) > ev.pageX){
						x = ev.pageX;
						this.volume -= 0.1
					}
	      ...
	      let x = e.pageX;
				...
				document.onmousemove = (ev)=>{
					if(x + 10 < ev.pageX){
						this.player.currentTime = this.player.duration * (ev.pageX / this.player.clientWidth);
						x = ev.pageX;
					}else if(x - 10 > ev.pageX){
						x = ev.pageX;
						this.player.currentTime = this.player.duration * (ev.pageX / this.player.clientWidth);
					}
				}
