<template>
  <div class="container-fluid">
                ...
                <tbody>
                  <tr v-for="(item, index) in data" :key="index">
                    ...
                    <td class="table__btn-container">
                      <input type="checkbox" ref="change" @change="(e) => {pushToDelete(e.target.checked, item)}">
                    </td>
                  </tr>
                </tbody>
              </table>
    ...
    <transition name="delete-fade">
      <div class="delete-btn" title="Удалить" @click="removeElementInData()" v-if="deleteMas.length">
        <fai icon="trash-alt"/>
      </div>
    </transition>
  </div>
...
export default {
  ...
  methods: {
    pushToDelete(target, data){
      if(target) 
        this.deleteMas.push(data);
      else
        for(let i = 0; i < this.deleteMas.length; i++)
          if(this.deleteMas[i] === data){
            this.deleteMas.splice(i, 1);
            break;
          }
    },
    getData(flag) {
      ...
      this.$http.post(`/api`, `method=GET-LOGS&filter=${this.filter}&limit=${this.limit}&id=${this.id}&order=${this.order}&text=${this.search}`)
      .then(response => {
        if (response.status == 200) {
          ...
          this.data = flag ? this.data.concat(response.data.data) : response.data.data;
    ...
    removeElementInData() {
      this.$swal({
        ...
        preConfirm: login => {
           return this.$http.post(`/api`, `method=DELETE-LOG&data=${JSON.stringify(this.deleteMas)}`)
        ...
        for(let i = 0; i < this.deleteMas.length; i++)
          for(let j = 0; j < this.data.length; j++)
            if(this.data[j] === this.deleteMas[i]){
              this.data.splice(j, 1);
              break;
            }
        for(let i = 0; i < this.$refs['change'].length; i++)
          this.$refs['change'][i].checked = false;
