...
export default {
  ...
  computed: {
    avaUrlComputed(){return this.student.ava.indexOf("/") == -1?`${this.$commonPhotoUrl}${this.student.ava}`:this.student.ava},
    studentSignUrlComputed() { return `${ this.$webSmartchatUrl }guide?code=${ this.student.guide }` }
  },
  methods: {
    closeDropdown() {
      console.log('popal')
    ...
    saveAllCharts() {
      ...
      if (!flag) {
        ...
        if (data.length) {
          this.$http.post(`/api`, `method=CREATE-OR-UPDATE-CHART&data=${JSON.stringify(data)}&&group_id=${this.learningProcessMainInfo.group_id}`)
          .then(async (response) => {
            if (response.data.status == 200) {
              ...
              this.learningProcessChartInfoOld = '';
               for (let i = 0; i < this.learningProcessChartInfo.length; i++) {
                  this.learningProcessChartInfo[i].lesson = data[i].lesson;
                  this.resetChartColor(i);
                  this.chartDisableEditing(i);
                }
              this.$swal({
                type: 'success',
                title: 'Успешно!',
    ...
    learningProcessChartInfoParse(data) {
      if (data.length) {
        for (let i = 0; i < data.length; i++) {
    ...
    getVisitHistory(flag) {
      if (!flag) this.visitHistoryFrom = 0;
        this.$http.post(`/api`, `method=VISIT-HISTORY&student_id=${this.student_id}&order=${this.visitHistoryOrderDesc}&from=${this.visitHistoryFrom}&to=${this.visitHistoryTo}`)
        .then(async (response) => {
          ...
          if (response.data.status == 200) {
            ...
            this.visitHistory = flag ? this.visitHistory.concat(response.data.visitHistory) : response.data.visitHistory;
    ...
    resetThePassword() {
      this.$swal({
        ...
        preConfirm: (login) => {
          return this.$http.post(`/api`, `method=RESET-PASSWORD&student_id=${ this.student_id }`)
    ...
    levelSaveEdit() {
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/api`, `method=CHANGE-LEVEL&student_id=${ this.student_id }&level_id=${ this.temp_level_id }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    toggleEmailOn(event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/api`, `method=EMAIL-ON&student_id=${ this.student_id }&value=${ this.student.mailOn ? 0 : 1 }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    toggleSmsOn(event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/api`, `method=SMS-ON&student_id=${ this.student_id }&value=${ this.student.smsOn ? 0 : 1 }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    toggleEmailActivated(event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/api`, `method=EMAIL-STATUS&student_id=${ this.student_id }&value=${ this.student.activated }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    togglePhoneActivated(event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/api`, `method=PHONE-STATUS&student_id=${ this.student_id }&value=${ this.student.is_active }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    toggleBlocked(event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: (r) => {
          return this.$http.post(`/api`, `method=BLOCK-USER&student_id=${ this.student_id }&block=${ this.student.blocked ? 0 : 1 }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    toggleStream(event) {
      ...
      this.$swal({
        ...
        showLoaderOnConfirm: true,
        preConfirm: () => {
          return this.$http.post(`/api`, `method=STREAM-STATE&student_id=${ this.student_id }&stream=${ this.student.stream ? 0 : 1 }`)
          .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
          .catch(e => { throw new Error(response.statusText); })
        },
        allowOutsideClick: () => !this.$swal.isLoading()
    ...
    getStudentAllInfo() {
      this.$http.post(`/api`, `method=GET-STUDENT&student_id=${this.student_id}`)
      .then(async (response) => {
        if (response.data.status == 200) {
          ...
          if(response.data.regSheduleInfo)this.regSheduleInfo = this.getClientInterval(response.data.regSheduleInfo);
    ...
    showEditRatePopup() {
      let vm = this;
      vm.$http.post(`/api`, `method=GET-RATES&activeRates=1`)
      .then(async (response) => {
        ...
        if (response.status == 200) {
            ...
            preConfirm: (key) => {
              if (key) { 
                if (+key == vm.learningProcessMainInfo.rate_id) {
                  ...
                  return vm.$http.post(`/api`, `method=CHANGE-RATE&rate_id=${key}&group_id=${vm.learningProcessMainInfo.group_id}`)
                  .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
                  .catch(e => { throw new Error(response.statusText); })
                }
              }
          ...
          .then((result) => {
            this.learningProcessMainInfo.rate_id = result.value;

            for(let i = 0; i < vm.GetSelectedRate.rate.length; i++){
                if(vm.GetSelectedRate.rate[i].rate_id == result.value){
                this.learningProcessMainInfo.rate_name = vm.GetSelectedRate.rate[i].rate_name;
                break;
              }
            }
    ...
    showEditRateTeacherPopup() {
      let vm = this;
      vm.$http.post(`/api`, `method=GET-TEACHERS&unblockedTeachers=1`)
      .then(async (response) => {
        let title = 'Все преподаватели';
        if (response.status == 200) {
            ...
            preConfirm: (key) => {
              if (key) { 
                if (+key == vm.learningProcessMainInfo.teacher_id) {
                  ...
                  return vm.$http.post(`/api`, `method=CHANGE-TEACHER&teacher_id=${key}&group_id=${vm.learningProcessMainInfo.group_id}`)
                  .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
                  .catch(e => { throw new Error(response.statusText); })
                }
              }
          ...
          .then((result) => {
            this.learningProcessMainInfo.teacher_id = result.value;

            for(let i = 0; i < vm.selectedTeacher.teacher.length; i++){
                if(vm.selectedTeacher.teacher[i].teacher_id == result.value){
                this.learningProcessMainInfo.teacher_name = vm.selectedTeacher.teacher[i].teacher_name;
                break;
              }
            }
            if (result.value && result.value !== -1) {
    ...
    showEditGroupPopup: async function () {
      let vm = this;
      vm.$http.post(`/api`, `method=GET-GROUPS&student_id=${vm.student_id}&all=${vm.selectSuitableGroup}`)
      .then(async (response) => {
        let footerText = '';
        let title = '';
            ...
            preConfirm: (key) => {
              if (key) { 
                if (+key == vm.learningProcessMainInfo.group_id) {
                  ...
                  return vm.$http.post(`/api`, `method=UPDATE-GROUP&group_id=${key}&student_id=${vm.student_id}`)
                  .then(response => { if (response.status != 200) { throw new Error(response.statusText); } })
                  .catch(e => { throw new Error(response.statusText); })
                }
              }
